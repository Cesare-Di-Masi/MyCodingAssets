...
namespace WeaponGenerator
{
    ...
    public sealed class WeaponDna
    {
        ...
        {
            get => Genes.TryGetValue(G_BaseForm, out var v) ? v.AsEnum<BaseFormType>() : BaseFormType.LongSword;
            set => Genes[G_BaseForm] = GeneValue.FromEnum((int)value);
        }
        ...
        // --------- Construction API ---------
        public static WeaponDna GenerateRandom(int targetGeneCount, Random? rng = null)
        {
            rng ??= new Random();
        ...
        public void RepairCoherence(Random? rng = null)
        {
            ...
            foreach (var issue in issues)
            {
                if (issue.Contains("beam requires"))
                {
                    ...
                    Ensure("channels.emission.mode", GeneValue.FromEnum((int)EmissionMode.Beam));
                    EnsureElement(rng);
                }
                else if (issue.Contains("projectile block"))
                {
                    Ensure("projectile.archetype", GeneValue.FromEnum((int)ProjectileArchetype.Arrow));
                    Ensure("projectile.ballistic", GeneValue.FromEnum((int)BallisticKind.Flat));
                    Ensure("projectile.muzzle_velocity_ms", GeneValue.Float(60));
                }
                else if (issue.Contains("trap/mine"))
                {
                    Ensure("projectile.ballistic", GeneValue.FromEnum((int)BallisticKind.Arced));
                ...
                else if (issue.Contains("conductivity vs shock"))
                {
                    ...
                    if (Genes.TryGetValue("materials.conductivity", out var cv) && cv.AsEnum<Conductivity>() == Conductivity.None)
                        Genes["materials.conductivity"] = GeneValue.FromEnum((int)Conductivity.Low);
                    else
                        Genes["channels.elements.shock.intensity"] = GeneValue.Float(0.4f);
                }
                else if (issue.Contains("grip/length/mass"))
                {
                    Ensure("class.grip", GeneValue.FromEnum((int)Grip.TwoHand));
                }
        ...
        {
            if (Genes.TryGetValue("delivery.primary", out var dp) && dp.AsEnum<DeliveryPrimary>() == DeliveryPrimary.Beam)
            {
                if (!Genes.TryGetValue("channels.emission.mode", out var em) || em.AsEnum<EmissionMode>() == EmissionMode.None)
        ...
        private void Rule_ProjectilesRequireProjectileBlock(List<string> issues)
        {
            ...
            {
                var p = dp.AsEnum<DeliveryPrimary>();
        ...
        private void Rule_TrapMineConstraints(List<string> issues)
        {
            if (Genes.TryGetValue("delivery.primary", out var dp))
            {
                ...
                {
                    if (!Genes.TryGetValue("projectile.ballistic", out var b) || b.AsEnum<BallisticKind>() == BallisticKind.Hitscan)
        ...
        {
            if (Genes.TryGetValue("materials.conductivity", out var c) && c.AsEnum<Conductivity>() == Conductivity.None)
            {
        ...
        {
            if (Genes.TryGetValue("class.grip", out var g) && g.AsEnum<Grip>() == Grip.TwoHand)
            {
                var ok = false;
        ...
        {
            if (Genes.TryGetValue("control.stagger", out var sg) && sg.AsEnum<Stagger>() == Stagger.Heavy)
            {
    ...
    public readonly struct GeneValue
    {
        ...
        private GeneValue(GeneKind kind, object value)
        ...
        public static GeneValue FromEnum(int v) => new(GeneKind.Enum, v);
        ...
        public TEnum AsEnum<TEnum>() where TEnum : struct, System.Enum
        => (TEnum)System.Enum.ToObject(typeof(TEnum), Convert.ToInt32(Value));
    ...
    public sealed class GeneValueJsonConverter : JsonConverter<GeneValue>
    {
        public override GeneValue Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
        {
            ...
            {
                JsonTokenType.Number => reader.TryGetInt32(out var i) ? GeneValue.FromEnum(i) : GeneValue.Float(reader.GetSingle()),
    ...
    public sealed class GeneBlueprint
    {
        ...
        public GeneBlueprint(string key, GeneKind kind)
        { Key = key; Kind = kind; }

        public GeneBlueprint Optional()
        { OptionalFlag = true; return this; }

        public GeneBlueprint WithRange(float min, float max)
        { MinF = min; MaxF = max; return this; }

        public GeneBlueprint WithRange(int min, int max)
        { MinI = min; MaxI = max; return this; }
        ...
        public GeneBlueprint WithEnumValues(params string[] _)
        { /* value names unused; indices suffice */ return this; }
        ...
        public GeneValue InstantiateRandom(Random rng)
        {
            return Kind switch
            {
                ...
                GeneKind.Enum => GeneValue.FromEnum(EnumValues.Length == 0 ? rng.Next(0, 8) : EnumValues[rng.Next(EnumValues.Length)]),
                _ => GeneValue.Int(0)
            };
        }
        ...
        public GeneValue Mutate(GeneValue current, Random rng)
        {
            return Kind switch
            {
                ...
                GeneKind.Enum => GeneValue.FromEnum(MutateEnum(current.AsInt(), EnumValues, rng)),
                _ => current
            };
        }
    ...
    public enum Grip
    { OneHand, TwoHand, Versatile, Dual }

    public enum DeliveryPrimary
    { Slash, Thrust, Crush, Cleave, Pierce, Slam, Projectile, Beam, Trap, Mine, Chain, Whip, Throw }

    public enum CoreMaterial
    { Steel, Iron, Obsidian, Bone, Crystal, Bronze, Tungsten, Ceramic, Wood, Composite, PlasmaCaged, Voidglass, Chitin, Graphene, LivingResin }

    public enum JacketMaterial
    { None, Steel, Bronze, Chitin, Graphene, Crystal, Ceramic, Composite, LivingResin }

    public enum Conductivity
    { None, Low, Medium, High }

    public enum ProjectileArchetype
    { Arrow, Bolt, Shard, Bomb, Dart, Javelin, Disk, Tether }

    public enum BallisticKind
    { Hitscan, Flat, Arced }

    public enum HomingKind
    { None, Mild, Strong }

    public enum EmissionMode
    { None, Beam, Pulse, Cone, Aura, ProjectileField }

    public enum Element
    { Phys, Fire, Frost, Shock, Acid, Rot, Bleed, Arcane, Gravity, Sonic, Radiation, Light, Shadow }

    public enum Stagger
    { None, Light, Medium, Heavy }

    public enum DotType
    { None, Bleed, Burn, Poison, Rot, Radiation }

    public enum AILevel
    { Inert, Reactive, SemiSentient, Sentient }

    public enum ModuleKind
    { Empty, Capacitor, Rune, Counterweight, Gyro, Converter, HeatSink, AutoLoader, Bayonet, NetLauncher, MiniShield }
